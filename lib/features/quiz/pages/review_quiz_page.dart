...
import '../../../models/review_quiz_models.dart';
...
class _ReviewQuizPageState extends State<ReviewQuizPage> {
  ...
  Future<List<QuizWord>> _fetchWordsByIds(List<String> ids) async {
    if (ids.isEmpty) return [];

    final List<QuizWord> result = [];
  ...
  Future<void> _loadReviewQuestions() async {
    ...
    try {
      ...
      final mistakesSnap = await FirebaseFirestore.instance
          .collection('users')
          .doc(user.uid)
          .collection('mistakes')
      ...
      final mistakes = mistakesSnap.docs
          .map((d) => Mistake.fromDoc(d))
          .where((m) => m.wordId.isNotEmpty)
          .toList();
      ...
      final ids = mistakes.map((m) => m.wordId).toSet().toList();
      final fetchedWords = await _fetchWordsByIds(ids);

      final wordMap = <String, QuizWord>{};
      for (final w in fetchedWords) {
        wordMap[w.id] = w;
      }

      final reviewWords = <QuizWord>[];
      for (final m in mistakes) {
        final fromDb = wordMap[m.wordId];
        if (fromDb != null) {
          reviewWords.add(fromDb);
        } else {
          reviewWords.add(
            QuizWord(
              id: m.wordId,
      ...
      final globalSnap =
      await FirebaseFirestore.instance.collection('words').limit(200).get();

      final globalWords = globalSnap.docs
      ...
      final built = QuizEngine.buildQuiz(
        ...
        // Review’de cümle soruları da mistakes’e yazılsın:
        trackMistakesForSentenceQuestions: true,
      );
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Review yüklenirken hata oluştu: $e')),
      );
    }
  }

  // --------------- MISTAKE + ANSWER LOGIC ---------------

  Future<void> _trackMistake({
  ...
  void _answerChoice(int index) async {
    ...
    await _trackMistake(q: q, isCorrect: isCorrect);
  }
  ...
  void _checkBuildAnswer() async {
    ...
    await _trackMistake(q: q, isCorrect: ok);
  }
        ...
        SnackBar(
          content: Text(
            '$total sorudan $_correctCount doğru. '
                '${_earned > 0 ? '$_earned puan kazandın! 🎉' : 'Bu reviewden puan kazanamadın.'}',
          ),
        ),
      );
    } catch (e) {
  ...
  void _retry() => _loadReviewQuestions();

  // ---------------- UI ----------------

  @override
  Widget build(BuildContext context) {
    const titleText = 'Review (Hatalar)';
        ...
        body: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const Text(
                'Review için hata bulunamadı 🎉\nYanlış yaptığın kelime olunca burada görünecek.',
              ...
              SizedBox(
                ...
                child: ElevatedButton.icon(
                  onPressed: () => context.go('/word-quiz'),
                  icon: const Icon(Icons.quiz),
                  label: const Text('Rastgele Quiz Çöz'),
                ),
              ),
            ...
            if (isBuild) ...[
              ...
              Wrap(
                ...
                children: remainingTokens
                    .map((t) => ActionChip(
                    label: Text(t.text), onPressed: () => _pickToken(t)))
                    .toList(),
              ),
            ] else ...[
